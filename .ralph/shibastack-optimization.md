...
- **Iteration P18 (test hardening — plateau):** added `testGetImagesPrefixStripVariants` (docker.io/library, docker.io/<user>/<repo>, and non-docker.io host preserved) and `testHostPortSkipsUnparseableMappings` (first parseable host port wins; non-numeric skipped, no crash). Test-only; mirrors verified fake-engine patterns; runs in CI/Xcode.
- **Iteration P19 (engine-status honesty):** a runtime-downtime episode revealed a real UX gap — when the Apple container apiserver is NOT running (it auto-stops when no containers run), `getContainers()` returns `[]`, so the UI showed an empty list indistinguishable from "no containers." Added `ContainerManager.isRuntimeRunning()` (parses `container system status` for "apiserver is running"); `GUIStateManager.runtimeRunning` is updated each refresh; the Containers view shows an honest yellow banner ("The container engine is not running. Start it with `container system start`") when down. Builds warning-free; status strings confirmed from the live runtime. (The earlier QA "failures" were this exact transient downtime — not a code bug.)
- **Status: STABLE PLATEAU.** App is feature- + honesty-complete. Remaining work needs external inputs (notarized signing — Apple Developer account) or is large/out-of-scope (Kubernetes, multi-distro machines, compose). Recommended pausing the loop; awaiting user direction or "stop".
