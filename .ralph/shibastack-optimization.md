...
- **Backlog (next iterations):** image-pull progress parsing; addPortForward no-op honesty + network status health checks (DNS:15353, proxy:8080); live stats in the container list (not just detail); the terminal exec path still routes through VSOCKManager→guest-vminitd (works in mock mode) — consider a direct `container exec` fallback for robustness.
