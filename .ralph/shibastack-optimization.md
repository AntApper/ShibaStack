...
- **Backlog (next iterations):** live stats in the container LIST (not just detail, mind exec cost); terminal exec robustness (VSOCKManager→guest-vminitd works in mock mode; consider direct `container exec` fallback); image build (`container build`) UI; per-container restart/stop-all polish; surface pull success/failure via list refresh diff.
